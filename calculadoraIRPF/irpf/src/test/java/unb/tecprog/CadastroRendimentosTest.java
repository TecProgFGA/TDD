...
public class CadastroRendimentosTest
{
    ...
    public void T01_CadastroUmRendimento() {
        simuladorIRPF.cadastrarRendimento(new Rendimento("Salario", 16000.0));
        assertEquals(16000.0, simuladorIRPF.getTotalRendimentos(), 0.0);
    }

    @Test
    public void T02_CadastroDoisRendimento() {
        simuladorIRPF.cadastrarRendimento(new Rendimento("Salario", 10000.0));
        simuladorIRPF.cadastrarRendimento(new Rendimento("Aluguel", 2000.0));
        assertEquals(12000.0, simuladorIRPF.getTotalRendimentos(), 0.0);
    }

    @Test
    public void T03_CadastroTresRendimento() {
        simuladorIRPF.cadastrarRendimento(new Rendimento("Salario Total", 12000.0));
        simuladorIRPF.cadastrarRendimento(new Rendimento("Aluguel", 2000.0));
        simuladorIRPF.cadastrarRendimento(new Rendimento("Rendimento de dividendos", 1000.0));
